...
export function atsScore(
  ...
  const structureHits = structureKeys.filter(k =>
    safeText.includes(k.toLowerCase())
  ).length;
  const structureScore = (structureHits / structureKeys.length) * 15;

  // 技能交集
  const skillHits = skills.filter(s =>
    safeText.includes((s || '').toLowerCase())
  ).length;
  ...
  const total =
    keywordScore + quantifyScore + structureScore + skillScore + concise;
