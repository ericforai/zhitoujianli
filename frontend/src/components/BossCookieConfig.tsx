...
const BossCookieConfig: React.FC<BossCookieConfigProps> = ({ onClose }) => {
  ...
  const fetchCookieStatus = async () => {
    try {
      const response = await axios.get(
        'http://115.190.182.95:8080/api/boss/cookie'
      );
  ...
  const handleStartBossWithUI = async () => {
    ...
    try {
      const response = await axios.post(
        'http://115.190.182.95:8080/start-boss-task-with-ui',
        {}
      );
      if (response.data.success) {
        setMessage(
          '✅ 有头模式登录已启动！请在弹出的浏览器窗口中完成登录，登录成功后会自动切换到无头模式。'
        );
      } else {
        setMessage('⚠️ ' + response.data.message);
      }
      ...
      setMessage(
        '❌ 有头模式启动失败：当前服务器环境不支持图形界面。请使用手动配置Cookie的方式。'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleHybridDelivery = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://115.190.182.95:8080/api/boss/start-hybrid-delivery?userId=${userId}`
      );
      ...
      if (response.data.success) {
        if (response.data.download_script) {
          // 需要下载脚本
          setMessage(
            '✅ 首次使用需要本地登录！请下载并运行脚本，在本地浏览器中完成登录。'
          );
      ...
      <div className='bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto'>
        ...
        <div className='mb-6 p-4 bg-blue-50 rounded-lg'>
          ...
          <ol className='text-sm text-blue-700 space-y-1 list-decimal list-inside'>
            ...
            <li>登录你的Boss直聘账号</li>
            <li>
              按F12打开开发者工具，切换到&quot;Application&quot;或&quot;应用程序&quot;标签
            </li>
            <li>
              在左侧找到&quot;Cookies&quot; → &quot;https://www.zhipin.com&quot;
            </li>
            <li>
              找到 <code className='bg-blue-100 px-1 rounded'>zp_token</code> 和{' '}
              <code className='bg-blue-100 px-1 rounded'>session</code>{' '}
              两个Cookie
            </li>
        ...
        <div className='mt-4 p-3 bg-gray-50 rounded-lg text-sm text-gray-600'>
          ...
          <ul className='list-disc list-inside space-y-1 mt-1'>
            <li>
              <strong>🚀 智能投递（推荐）</strong>
              ：首次使用会下载脚本到本地，在本地浏览器中扫码登录，登录成功后自动切换到无头模式
            </li>
            <li>
              <strong>启动Boss程序</strong>
              ：使用已保存的Cookie直接启动，无需登录
            </li>
            <li>
              <strong>手动配置Cookie</strong>
              ：如果不想使用本地脚本，可以手动获取Cookie并配置
            </li>
          </ul>
        </div>
