...
const DeliveryControl: React.FC<DeliveryControlProps> = ({
  ...
  // 当前是否正在投递（本地或服务器）
  const isDelivering = deliveryMode === 'local' ? isLocalDelivering : serverStatus.isRunning;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">选择投递方式，开始自动投递简历</p>
            </div>
          </div>
              ...
              <div className="flex-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-3 p-3 bg-white rounded-lg border border-amber-200">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-2">下载后解压到任意位置</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-white rounded-lg border border-amber-200">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-amber-900 font-medium mb-1">
                        双击运行 <code className="bg-amber-100 px-1.5 py-0.5 rounded text-amber-800">start_agent.py</code>
                      </p>
                      <p className="text-xs text-gray-600">
                        输入智投简历账号密码 → 扫码登录Boss直聘 → 回来点「开始投递」
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 p-2 bg-gray-100 rounded-lg">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-xs text-gray-400">（连接成功后此处会变绿）</span>
                  </div>
                </div>
            ...
            <>
              ...
              <div className="text-sm text-gray-500">
                {deliveryMode === 'local' ? (
                  ...
                  <span>今日已投递: <span className="font-semibold text-gray-900">{serverStatus.deliveryCount || 0}</span> 个</span>
                )}
              </div>
            </>
          ) : (
        ...
        {(serverMessage || localDeliveryMessage) && (
          <div
            className={`mt-4 p-3 rounded-lg text-sm ${
              (serverMessage || localDeliveryMessage || '').includes('失败') || (serverMessage || localDeliveryMessage || '').includes('错误') || (serverMessage || localDeliveryMessage || '').includes('❌')
                ? 'bg-red-50 text-red-700 border border-red-200'
                : (serverMessage || localDeliveryMessage || '').includes('成功') || (serverMessage || localDeliveryMessage || '').includes('✅') || (serverMessage || localDeliveryMessage || '').includes('已下发')
