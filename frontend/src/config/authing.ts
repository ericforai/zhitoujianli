...
const AUTHING_CONFIG = {
  ...
  // 重定向URI - 登录成功后的跳转地址
  redirectUri:
    process.env.REACT_APP_AUTHING_REDIRECT_URI ||
    window.location.origin + '/login',

  // 登出重定向URI
  logoutRedirectUri:
    process.env.REACT_APP_AUTHING_LOGOUT_REDIRECT_URI ||
    window.location.origin + '/login',
...
export const createAuthingClient = (): AuthenticationClient => {
  try {
    ...
    if (!AUTHING_CONFIG.appId) {
      throw new Error(
        'AUTHING_APP_ID未配置，请在.env文件中设置REACT_APP_AUTHING_APP_ID'
      );
    }

    if (!AUTHING_CONFIG.appHost) {
      throw new Error(
        'AUTHING_APP_HOST未配置，请在.env文件中设置REACT_APP_AUTHING_APP_HOST'
      );
    }
