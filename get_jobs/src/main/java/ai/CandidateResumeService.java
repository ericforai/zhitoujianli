...
public class CandidateResumeService {
    ...
    private static final String CANDIDATE_RESUME_PATH = "src/main/resources/candidate_resume.json";
    ...
    private static void saveCandidateInfo(Map<String, Object> candidate) {
        try {
            // 确保目录存在
            File file = new File(CANDIDATE_RESUME_PATH);
            ...
            log.info("【简历解析】候选人信息已保存到: {}", CANDIDATE_RESUME_PATH);
    ...
    public static Map<String, Object> loadCandidateInfo() {
        try {
            File file = new File(CANDIDATE_RESUME_PATH);
            if (!file.exists()) {
                log.warn("【简历解析】候选人简历文件不存在: {}", CANDIDATE_RESUME_PATH);
                return null;
            }
            
            String jsonString = Files.readString(Paths.get(CANDIDATE_RESUME_PATH));
            ObjectMapper mapper = new ObjectMapper();
    ...
    public static void deleteCandidateResume() {
        try {
            ...
            if (file.exists()) {
                file.delete();
                log.info("【简历解析】已删除候选人简历");
            }
        } catch (Exception e) {
            log.error("【简历解析】删除候选人简历失败", e);
        }
    }
