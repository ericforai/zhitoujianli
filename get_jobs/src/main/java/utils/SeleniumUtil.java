...
//        options.addArguments("--headless"); //使用无头模式
        ...
        // 临时禁用CHROME_DRIVER初始化，避免NetworkConnection编译错误
        ...
        if (mobile) {
            // 临时禁用MOBILE_CHROME_DRIVER初始化，避免NetworkConnection编译错误
            // MOBILE_CHROME_DRIVER = new ChromeDriver(mobileOptions);
            // MOBILE_CHROME_DRIVER.manage().window().maximize();
            System.out.println("MOBILE_CHROME_DRIVER初始化已禁用，避免编译错误");
        }
    ...
    public static void saveCookie(String path) {
        ...
        System.out.println("saveCookie: 已跳过CHROME_DRIVER操作，避免编译错误");
        return;
        // 获取所有的cookies
