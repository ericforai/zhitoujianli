...
class AuthService:
    ...
    async def login(self, email: str, password: str) -> bool:
        ...
        try:
            ...
            ) if certifi else ssl.create_default_context()
            connector = aiohttp.TCPConnector(ssl=ssl_context)

            async with aiohttp.ClientSession(connector=connector) as session:
                # 与前端 AuthContext / Spring AuthController 一致：邮箱登录走 /api/auth/login/email
                login_url = f"{self.config.api_url.rstrip('/')}/api/auth/login/email"
                payload = {
                    "email": email,
                    "password": password,
                }
            ...
            logger.error(f"❌ 证书校验失败: {e}")
            logger.error("请执行: pip install --upgrade certifi aiohttp")
            logger.error("macOS 还需执行 Python 的 Install Certificates.command")
            return False
            ...
            self.last_error_type = "exception"
    ...
    async def connect(self):
        ...
        try:
            ...
            if "InvalidStatusCode" in error_type or "status_code" in error_msg.lower():
                ...
                token_preview = self.config.access_token[:8] if self.config.access_token and len(self.config.access_token) > 8 else '***'
                logger.error(f"连接URL: {self.config.server_url}?token={token_preview}...")
...
def main():
    ...
    parser.add_argument(
        "--server", "-s",
        default=None,
        help="WebSocket服务器地址（不指定则自动检测本地/生产环境）"
    )
    parser.add_argument(
        "--api", "-a",
        default=None,
        help="API服务器地址（不指定则自动检测本地/生产环境）"
    )
    parser.add_argument(
        "--token", "-t",
        default=None,
        help="网站「本地 Agent」生成的 Agent Token（与邮箱密码登录二选一）"
    )
    parser.add_argument(
        "--dev",
        action="store_true",
        help="开发模式（默认本地后端 localhost:8080，与 Spring server.port 一致）"
    )
    parser.add_argument(
        "--headless",
        action="store_true",
    ...
    # 确定服务器地址（--dev 默认 8080；可用 --server / --api 覆盖）
    if args.dev:
        server_url = "ws://localhost:8080/ws/local-agent"
        api_url = "http://localhost:8080"
        logger.info("🔧 开发模式: 默认本地后端 localhost:8080")
    else:
        server_url = args.server or "wss://www.zhitoujianli.com/ws/local-agent"
        api_url = args.api or "https://www.zhitoujianli.com"
    ...
    agent_token = (args.token or "").strip() if args.token else ""
    ...
    if agent_token:
        ...
        if not email:
            ...
            print("请输入您的智投简历账号（或改用: python3 boss_local_agent.py --token <Token>）")
            print("=" * 50 + "\n")
        ...
        if not email or not password:
            logger.error("❌ 请提供邮箱和密码，或使用 --token")
            sys.exit(1)
        ...
        async def run_agent():
            ...
            while attempt < max_retries:
                ...
                error_type = agent.auth_service.last_error_type
                if error_type in ("ssl", "network"):
                    logger.error("❌ 当前是网络/证书问题，本次不计入密码错误次数")
                    logger.error("建议先修复证书: pip install --upgrade certifi aiohttp")
                    return
